/* pro-script.js - V260.0 (Asset Double Count Fix) */
...
/* --- [핵심 수정] 시스템 시작 및 자산 복구 --- */
function startSystem(s=false) {
    // 1. 잔고 체크 (자동 충전 로직 제거 - 중복 방지)
    if (appState.balance < 10) {
        // 지갑 페이지가 없으면(일반 버전) 충전해주지만, 있으면 경고만
        const h = document.querySelector('a[href="wallet.html"]');
        if (!h && appState.balance === 0) { 
            if(!s) alert("테스트 자금 충전 ($1,000)"); 
            appState.balance += 1000; 
        } else if (h) {
            if(!s) alert("지갑 잔고가 부족합니다. 입출금 메뉴를 이용하세요."); 
            stopSystem(true); 
            return;
        }
    }

    if (!appState.config.isReady) { if(!s) return; return; }

    // 2. 변수 복구
    appState.runningCoin = appState.config.target;
    
    // [중요] 포지션이 이미 있다면, 투자금액을 포지션 기준으로 강제 맞춤 (뻥튀기 방지)
    if (appState.position) {
        appState.investedAmount = appState.position.entryPrice * appState.position.quantity;
    } else {
...
function renderGlobalUI() {
    ...
    // 1. 메인 총 자산 표시
    if(elTotal) {
        let totalVal = appState.balance; // 기본값
        ...
        if(appState.isRunning && appState.position) {
            ...
            const positionValue = currentPrice * appState.position.quantity;
            
            // [공식 수정] (전체잔고 - 투자원금) + 현재평가금액
            // 투자원금이 0으로 잘못 되어있으면 여기서 2배가 됨. 위 startSystem에서 방어함.
            totalVal = (appState.balance - appState.investedAmount) + positionValue;
        }
        
        elTotal.innerText = `$ ${totalVal.toLocaleString(undefined, {minimumFractionDigits:2})}`;
        
        if(elProf) {
            const p = appState.dailyTotalProfit; // 누적 수익금
            // 수익률 계산 (시작 원금 기준)
...
function initInfoPage(c){try{if(typeof TradingView!=='undefined'){new TradingView.widget({"container_id":"info_tv_chart","symbol":`BINANCE:${c}USDT`,"interval":"15","theme":"dark","style":"1","locale":"kr","autosize":true,"hide_side_toolbar":false})}}catch(e){}startPriceStream(c);loadNewsData(c);updateInfoUI(68000)}
