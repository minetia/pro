/* pro-script.js - V42.0 (Strict Logic & UI Fixes) */
let appState = {
...
const SAVE_KEY = 'neuroBotData_V42_FIX';
const CONFIG_KEY = 'neuroConfig_V42_FIX';
...
function startSystem(isSilent = false) {
    if (appState.balance <= 0) {
        if(!isSilent) alert("⚠️ 지갑 잔고가 0원입니다. [입출금] 메뉴에서 입금해주세요.");
        stopSystem(true); return;
    }
    if (!appState.config || !appState.config.isReady) {
        if(!isSilent) {
            if(confirm("⚠️ AI 설정이 필요합니다. 이동하시겠습니까?")) window.location.href = 'ai-core.html';
        }
        stopSystem(true); return;
    }

    // [핵심 수정] 설정 금액 검증 (Strict Mode)
    let setAmount = appState.config.amount || 0;
    
    // 설정된 금액이 현재 내 지갑 잔고보다 크면 -> 실행 불가!
    if (setAmount > appState.balance) {
        if(!isSilent) {
            alert(`⛔ [실행 불가]\n설정 금액($${setAmount})이 보유 잔고($${appState.balance})보다 큽니다.\n\nAI설정에서 금액을 낮추거나 입금해주세요.`);
        }
        stopSystem(true); 
        return; // 여기서 강제 종료
    }
...
function renderGlobalUI() {
    ...
    // 현금: 실행 중이면 cash변수, 아니면 전체 balance
        ...
        if(appState.tradeHistory.length === 0) {
            ...
            appState.tradeHistory.slice(0, 50).forEach(t => {
                ...
                <div class="ledger-row">
                    <div class="col-1" style="color:#888;">${t.time}</div>
                    <div class="col-2" style="font-weight:bold;">${t.coin} <span class="${pnlColor}" style="font-size:0.7rem;">${t.type}</span></div>
                    <div class="col-3">${t.price}</div>
                    <div class="col-4">${t.net}</div>
                </div>`;
            });
    ...
    if(document.getElementById('holdings-list')) {
        ...
        if(pie) {
            // 투자중이면 노란색+회색, 현금만 있으면 회색 100%
            pie.style.background = `conic-gradient(var(--accent) 0% ${investPercent}%, #333 ${investPercent}% 100%)`;
        }
        ...
        if(appState.isRunning && invested > 0) {
            hHtml += `
                <div style="display:flex; justify-content:space-between; padding:12px 5px; border-bottom:1px solid #333;">
                    <div>
                        <div style="font-weight:bold; color:#fff; font-size:0.9rem;">${appState.runningCoin}</div>
                        <div style="font-size:0.7rem; color:var(--accent);">AI TRADING</div>
                    </div>
                    <div style="text-align:right;">
                        <div style="color:#fff;">$${invested.toLocaleString(undefined,{maximumFractionDigits:2})}</div>
        ...
        hHtml += `
            <div style="display:flex; justify-content:space-between; padding:12px 5px; border-bottom:1px solid #333;">
                <div>
                    <div style="font-weight:bold; color:#fff; font-size:0.9rem;">USDT</div>
                    <div style="font-size:0.7rem; color:#888;">현금 자산</div>
                </div>
                <div style="text-align:right;">
                    <div style="color:#fff;">$${currentCash.toLocaleString(undefined,{maximumFractionDigits:2})}</div>
    ...
    if(historyTable) {
        ...
        appState.tradeHistory.slice(0, 30).forEach(t => {
            ...
            tHtml += `<tr>
                ...
                <td>${t.vol}</td>
                <td>${t.total}</td> <td><span style="font-weight:bold;">${t.net}</span></td>
            </tr>`;
        });
    ...
    if(bankList && appState.transfers) { 
        ...
        appState.transfers.forEach(t => { 
            ...
            bHtml += `<div class="ledger-row">
                <div style="width:30%; font-size:0.8rem;">${t.date}</div>
                <div style="width:30%; font-weight:bold;" class="${color}">${t.type}</div>
                <div style="width:40%; text-align:right;">$${t.amount.toLocaleString()}</div>
            </div>`; 
        }); 
