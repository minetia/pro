/* pro-script.js - V26.0 (버튼 충돌 해결 & 잔돈 0원 보장) */
let appState = {
...
function openModal(mode) {
    isTransactionPending = true; // 거래 중엔 트레이딩 멈춤 (잔고 고정)
...
function calcPercent(percent) {
    ...
    if (percent === 100) input.value = baseAmount; // MAX는 있는 그대로
    else input.value = Math.floor(baseAmount * (percent / 100) * 100) / 100;
}

function closeModal() { 
    document.getElementById('transaction-modal').style.display = 'none'; 
    isTransactionPending = false; // 트레이딩 재개
}
...
function processTransaction(amount) {
    ...
    if (amount > 0) { // 입금
        ...
        if(appState.balance < 0.000001) appState.balance = 0; // 잔돈 0원 만들기
...
// [중요 수정] 메인 화면의 버튼만 찾아서 바꿈 (지갑 버튼 안 건드림)
function updateButtonState(isOn) {
    const btn = document.getElementById('btn-main-control'); // ID로 특정
    if(btn) {
...
function executeAiTrade(config) {
    ...
    const now = new Date();
    appState.tradeHistory.unshift({ date: `${now.getMonth()+1}/${now.getDate()}`, time: now.toLocaleTimeString('en-US',{hour12:false}), pos: positionLabel, in: currentPrice, profit: pnl });
...
function getRealisticPrice(symbol) {
    ...
    switch(symbol) {
        case 'BTC': return 96000 + (jitter * 500); case 'ETH': return 2700 + (jitter * 20); case 'SOL': return 180 + (jitter * 5); case 'XRP': return 2.4 + (jitter * 0.1); case 'DOGE': return 0.28 + (jitter * 0.01); default: return 100 + (jitter * 10);
    }
}
