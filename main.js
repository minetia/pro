...
window.addEventListener('load', function() {
    ...
    loadChartLibrary();     // 3. 차트 라이브러리 및 데이터 실행// [초기화] 페이지 로드 시 실행
window.addEventListener('load', function() {
    loadData();             // 1. 저장된 데이터(잔고, 내역) 불러오기
    createChartContainer(); // 2. 차트 박스 생성
    loadChartLibrary();     // 3. 차트 라이브러리 실행
    createOrderUI();        // 4. UI 그리기
    updateDashboard(currentPrice); // 5. 대시보드 갱신
    updateHistoryUI();      // 6. 체결 내역 갱신
});
...
function createChartContainer() {
    ...
    if (!container) {
        ...
        container.innerHTML = '<div style="position:absolute; top:50%; left:50%; transform:translate(-50%, -50%); color:#888;">📊 차트 데이터 로딩...</div>';
...
function initChart() {
    ...
    fetch('https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=1m&limit=100')
        ...
        .then(data => {
            ...
            updateMyPriceLine(); // ★ 새로고침 후에도 평단가 선 복구
            connectBinance(); 
        });
}
        ...
        <div style="background: #1e1e1e; border: 1px solid #333; border-radius: 12px; padding: 15px; margin: 10px;">
            <div style="display:flex; justify-content:space-between; margin-bottom:15px; border-bottom:1px solid #333; padding-bottom:10px;">
                ...
                <div style="text-align:right;">
                    ...
                    <div><span id="pnl-val" style="color:#fff; font-weight:bold;">$ 0.00</span> <span id="pnl-pct" style="font-size:12px;">(0.00%)</span></div>
                </div>
            </div>
            <div style="display:flex; gap:10px; margin-bottom:15px;">
                <input type="number" id="inp-price" placeholder="시장가(빈칸)" style="flex:1; padding:10px; background:#2a2a2a; border:1px solid #444; color:#fff; border-radius:6px;">
                <input type="number" id="inp-amount" placeholder="수량(BTC)" style="flex:1; padding:10px; background:#2a2a2a; border:1px solid #444; color:#fff; border-radius:6px;">
            </div>
            ...
            <button onclick="resetApp()" style="width:100%; margin-top:10px; background:transparent; border:none; color:#555; font-size:10px; text-decoration:underline; cursor:pointer;">데이터 초기화 (리셋)</button>
        </div>
...
window.order = function(side) {
    ...
    if (!pVal) {
        ...
        window.appState.pendingOrders.push({ id: Date.now(), side: side, price: parseFloat(pVal), amount: amount });
        saveData();
        updatePendingUI();
    }
};

function executeTrade(side, amount, price) {// [초기화] 페이지 로드 시 실행
window.addEventListener('load', function() {
    loadData();             // 1. 저장된 데이터(잔고, 내역) 불러오기
    createChartContainer(); // 2. 차트 박스 생성
    loadChartLibrary();     // 3. 차트 라이브러리 실행
...
function executeTrade(side, amount, price) {
    ...
    if(side === 'buy') {
        ...
        pos.entryPrice = ((pos.amount * pos.entryPrice) + (amount * price)) / (pos.amount + amount);
        pos.amount += amount;
        addHistory("매수", price, amount);
    } else {
        if(pos.amount < amount) return alert("코인 부족!");
        window.appState.balance += (amount * price);
        pos.amount -= amount;
        if(pos.amount <= 0) pos.entryPrice = 0;
        addHistory("매도", price, amount);
    }
    saveData();
    updateMyPriceLine();
    updateDashboard(currentPrice);
...
function addHistory(type, price, amount) {
    window.appState.tradeHistory.unshift({ time: new Date().toLocaleTimeString(), type: type, price: price, amount: amount });
    updateHistoryUI();
}
...
function updateHistoryUI() {
    ...
    window.appState.tradeHistory.forEach(h => {
        ...
        html += `<div style="display:flex; justify-content:space-between; padding:8px; border-bottom:1px solid #222; font-size:12px;">
            <span style="color:#666;">${h.time}</span> <span style="color:${color}; font-weight:bold;">${h.type}</span>
            <span style="color:#fff;">$${h.price.toLocaleString()}</span> <span style="color:#ccc;">${h.amount} BTC</span>
        </div>`;
    });
...
function updateMyPriceLine() {
    ...
    var pos = window.appState.position;
    if (pos && pos.amount > 0) {
        myPriceLine = candleSeries.createPriceLine({ price: pos.entryPrice, color: '#F0B90B', lineWidth: 2, lineStyle: 2, axisLabelVisible: true, title: '평단가' });
    }
}
...
function updatePendingUI() {
    ...
    window.appState.pendingOrders.forEach(o => {
        html += `<div style="padding:10px; background:#222; border-radius:4px; margin-bottom:5px; font-size:12px; color:#fff;">${o.side === 'buy' ? '매수' : '매도'} | $${o.price} | ${o.amount} BTC</div>`;
    });
    area.innerHTML = html;
}

window.resetApp = function() {
    if(confirm("모든 데이터를 초기화하시겠습니까?")) { localStorage.removeItem('tradingAppState'); location.reload(); }
};
